...
#[cfg(proc_macro_span)]
fn anchor_source_path() -> Option<std::path::PathBuf> {
    Some(proc_macro2::Span::call_site().source_file().path())
}
    ...
    match ty {
        ...
        syn::Type::Path(path) => {
            ...
            #[cfg(procmacro2_semver_exempt)]
            {
                ...
                let source_path = anchor_source_path()
                    .unwrap_or_else(|| std::env::current_dir().unwrap_or_default());
                let lib_path = find_path("lib.rs", &source_path)
                    .unwrap_or_else(|_| source_path.join("src").join("lib.rs"));
